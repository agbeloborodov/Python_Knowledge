...
grumpy = input()
grumpy_format = grumpy.lower().split()
grumpy_size = lambda x: sum(1 for i in x if i in 'аеёиоуыэюя')
grumpy_first_size = grumpy_size(grumpy_format[0])
if all([grumpy_size(i) == grumpy_first_size for i in grumpy_format]):
    print('Парам пам-пам  (годная, кошерная ворчалка)')
else:
    print('Пам парам (Винни, Винни, что с тобой?)')